...
export default class Stream<T extends any, A extends T> {
  ...
  constructor(func: (...args: NoNever<A>[]) => T, limit = INITIAL_LIMIT) {
    this.streamer = func
    this.#limit = limit
  }
   ...
   * });
   */
  forEach<D extends A>(initialArgument?: D): Runner<T>
  forEach<D extends A>(
  ...
  setSkip(n: number | undefined): Stream<T, A> {
    ...
    if (n && n >= MAX_LIMIT) {
      throw new Error(`Number most not be greater than ${MAX_LIMIT}`)
    }
    this.#skip = n
    return this
  }
  ...
  setLimit(number: number) {
    if (number > MAX_LIMIT) {
      throw new Error(`Limit most not be greater than ${MAX_LIMIT}`)
    }
    if (number <= 0) {
