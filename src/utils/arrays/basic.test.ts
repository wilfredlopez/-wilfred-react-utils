...
describe('Array Utils', () => {
  ...
  describe('mergeSort', () => {
    it('Returns the sorted array of string and numbers', () => {
      ...
      const expectedAsc = ['a', 'b', 'c', 'd', 'y', 'z']
      const expectedNs = [1, 2, 50, 100, 200, 400, 500]

      const sortedNumbers = mergeSort(testNs)
      const sortedStrings = mergeSort(testStr, alphabeticallySort)
    ...
    it('can sort in place', () => {
      const inPlaceSorted = [9, 29, 9, 1]
      expect(mergeSort(inPlaceSorted, null, true)).toBe(inPlaceSorted)
    })
  })
