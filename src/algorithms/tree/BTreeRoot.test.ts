import { BTreeRoot } from "./WBinarySearchTree"
...
let bst: BTreeRoot<DataType> | null = null

describe.skip("BTreeRoot", () => {
  beforeEach(() => {
    //head 20
    bst = new BTreeRoot<DataType>()
    bst.insert({
      priority: 20,
      data: "20",
    })
  ...
  it("Doesnt add if the priority is the same and 2nd parameter set to false", () => {
    ...
    expect(bst.root.right.right.value).toEqual({ priority: 50, data: "50" })
  })
  it("Replaces the value if the priority is the same and 2nd parameter is true", () => {
    ...
    expect(bst.root.right.right.value).toEqual({ priority: 50, data: "NEW 50" })
  })
  it("creates a bst and inserts the value in the correct place", () => {
    ...
    // expect(bst.right.right.value).toEqual({ priority: 50, data: "50" })
    expect(bst.root.value).toEqual({ priority: 20, data: "20" })
    expect(bst.root.left.value).toEqual({ priority: 1, data: "1" })
    expect(bst.root.left.right.value).toEqual({ priority: 2, data: "2" })
    expect(bst.root.right.value).toEqual({ priority: 21, data: "21" })
    expect(bst.root.right.right.value).toEqual({ priority: 50, data: "50" })
  })
