...
describe.skip("BinarySearchTree", () => {
  ...
  it("Works with for of loops", () => {
    const tree = new WBinarySearchTree<DataType>({
      priority: 20,
      data: "20",
    });
    //right 21
    tree.insert({ priority: 21, data: "21" });
    //right.right
    tree.insert({ priority: 50, data: "50" });
    //left
    tree.insert({ priority: 1, data: "1" });
    //left.left
    tree.insert({ priority: 2, data: "2" });

    const expectedVals = [];
    for (let val of tree) {
      expectedVals.push(val);
    }
    expect(expectedVals).toStrictEqual(
      [
        { priority: 20, data: "20" },
        { priority: 1, data: "1" },
        { priority: 21, data: "21" },
