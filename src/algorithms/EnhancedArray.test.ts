...
describe("Enhanced Array", () => {
  ...
  describe("Changing Default Value", () => {
    it("default value changed using setReplacerValue.", () => {
      const array = new EnhancedArray<number>();
      expect(array[200]).toEqual([]);
      array.setUndefinedReplacer(0);
      expect(array[200]).toBe(0);
      expect(array.getUndefinedReplacer()).toBe(0);
    });
  });
  describe("Deletes", () => {
